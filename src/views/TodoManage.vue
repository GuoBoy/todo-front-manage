<template>
  ...
  <el-table :data="filterUsers" style="width: 100%" size="small">
    ...
    <el-table-column prop="userid" label="UserID" width="66" align="center" />
    <el-table-column prop="detail" label="Detail" />
    <el-table-column prop="update" label="Update" sortable />
...
import { apiDelUser, apiGetAllToDoItems, apiUpdateUser } from '@/api';
import { ToDoItemModel } from '@/model/todo_item';
import { OPPwdModel } from '@/model/user';
import { ElMessage, ElMessageBox, FormInstance } from 'element-plus';
import { computed, onMounted, reactive, ref } from 'vue';

const items = ref<ToDoItemModel[]>([])
const onRefresh = () => {
  apiGetAllToDoItems().then((res: any) => {
    if (res.code !== 200) {
...
const filterUsers = computed(() => {
  return items.value.filter((v) => !searchKey.value || v.detail.includes(searchKey.value))
})
